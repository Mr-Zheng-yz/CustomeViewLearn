    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for ((index, child) in children.withIndex()) {
            ...
            if (index >= childrenBounds.size) {
                childrenBounds.add(Rect(widthUsed, heightUsed, widthUsed + child.measuredWidth, child.measuredHeight))
            } else {
                childrenBounds[index].set(widthUsed, heightUsed, widthUsed + child.measuredWidth, child.measuredHeight)
            }
        ...
        //确定自己宽高
        val selfWidth = resolveSizeAndState(widthUsed, widthMeasureSpec, 0)
        val selfHeight = resolveSizeAndState(heightUsed, heightMeasureSpec, 0)
