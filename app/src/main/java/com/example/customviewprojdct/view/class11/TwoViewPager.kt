...
class TwoViewPager @JvmOverloads constructor(
    context: Context,
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                val scrollX = scrollX
                val targetPage = if (abs(vx) < minVelocity) { //不是快滑
                    if (scrollX > width / 2) 1 else 0
                } else { //快滑，vx 大于0是负方向滑动；小于0是正方向滑动。
                    if (vx < 0) 1 else 0
                }
                val scrollDistanceX = if (targetPage == 1) width - scrollX else -scrollX
                Log.i("yanze", "本次滑动距离为：${scrollX} 距离目标页距离：${scrollDistanceX} 滑动速度为：${vx} 最大速度：${maxVelocity} 最小速度：$minVelocity")
                //计算滑动到终点所需时间
                overScroller.startScroll(getScrollX(), 0, scrollDistanceX, 0,1000)
